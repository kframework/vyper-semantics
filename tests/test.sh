...
kompile $SEMTANTICS_DIR/viper-lll.k --syntax-module VIPER-ABSTRACT-SYNTAX --debug -d .

echo "running tests/features/test_assignment/test_assignment.v"
krun $SCRIPT_DIR/features/test_assignment/test_assignment.v --output nowrap --debug | sed 's/.*<lll> \(.*\) <\/lll>.*/\1/' | sed 's/ , .LLLExps//g' > /tmp/viper.txt
diff /tmp/viper.txt $SCRIPT_DIR/features/test_assignment/test_assignment.v.out

echo "running tests/examples/token/ERC20.v"
krun $SCRIPT_DIR/examples/token/ERC20.v --output nowrap --debug | sed 's/.*<lll> \(.*\) <\/lll>.*/\1/' | sed 's/ , .LLLExps//g' > /tmp/viper.txt
